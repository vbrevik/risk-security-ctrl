...
/// Target chart display width in pixels (50% of the 800px originals).
const CHART_DISPLAY_WIDTH_PX: u32 = 400;
...
fn add_framework_section(
    ...
    // Column widths in twips. Total ~8900 fits within A4 margins (~6.2 inches usable).
    let table = Table::new(rows).set_grid(vec![1000, 2200, 1200, 900, 1100, 2500]);
...
/// Read width and height from PNG IHDR chunk (bytes 16..24 of a valid PNG).
...
fn embed_chart_image(docx: Docx, png_bytes: &[u8]) -> Docx {
    let (display_w, display_h) = match png_dimensions(png_bytes) {
        Some((w, h)) => {
            // Scale proportionally so display width = CHART_DISPLAY_WIDTH_PX
            let scale = CHART_DISPLAY_WIDTH_PX as f64 / w as f64;
            let scaled_h = (h as f64 * scale) as u32;
            (CHART_DISPLAY_WIDTH_PX, scaled_h)
        }
        None => (CHART_DISPLAY_WIDTH_PX, 300), // fallback 400x300
    };

    let pic = Pic::new(png_bytes).size(
        display_w * EMU_PER_PX,
        display_h * EMU_PER_PX,
    );
